...
function Field({
  label, type = "text", value, onChange, placeholder, required, error, showToggle, children,
}: {
  label: string; type?: string; value: string; onChange: (v: string) => void;
  placeholder?: string; required?: boolean; error?: string;
  showToggle?: boolean; children?: React.ReactNode;
}) {
  ...
  return (
    <div className="space-y-1.5">
      <label className="block text-xs font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>
        {label}
      </label>
      {children ?? (
...
function SignInForm({ onSwitch }: { onSwitch: (t: Tab) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isCred = msg.toLowerCase().includes("invalid login") || msg.toLowerCase().includes("invalid credentials");
      setFormError(isCred ? "Wrong email or password." : msg || "Sign in failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    catch (err) { toast.error(err instanceof Error ? err.message : "Google sign-in failed"); setGLoading(false); }
  };
  ...
  return (
    <motion.div key="signin" {...slide}>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5">
        ...
        <Field
          label="Password" type="password" value={password} showToggle
          onChange={(v) => { setPassword(v); if (formError) setFormError(""); }}
          placeholder="••••••••" required
        />

        {formError && (
          <p className="text-xs text-center py-2 px-3 rounded-lg" style={{ color: "#f87171", background: "rgba(239,68,68,0.08)", border: "0.5px solid rgba(239,68,68,0.2)" }}>
            {formError}
          </p>
        )}
        ...
        <div className="flex justify-end">
          <button type="button" onClick={() => onSwitch("forgot")}
            className="text-xs transition-colors" style={{ color: "#38E0C3" }}>
            Forgot password?
          </button>
        </div>
      ...
      <p className="text-center text-sm mt-6" style={{ color: "rgba(255,255,255,0.4)" }}>
        No account?{" "}
        <button onClick={() => onSwitch("signup")} className="font-medium" style={{ color: "#38E0C3" }}>
          Sign up free
        </button>
      </p>
      {bannedInfo && (
...
function SignUpForm({ onSwitch }: { onSwitch: (t: Tab) => void }) {
  ...
  const strengthColor = ["", "#ef4444", "#f59e0b", "#38E0C3", "#10b981"][strength];

  return (
    <motion.div key="signup" {...slide}>
      <h2 className="text-[22px] font-semibold text-white mb-1">Create account</h2>
      <p className="text-sm mb-6" style={{ color: "rgba(255,255,255,0.45)" }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5">
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>
            University <span style={{ color: "rgba(255,255,255,0.25)" }}>(optional)</span>
          </label>
          <select
            value={uni}
          ...
          >
            ...
            {NIGERIAN_UNIVERSITIES.map((u) => (
              <option key={u} value={u} style={{ background: "#0f2039", color: "white" }}>{u}</option>
            ))}
          </select>
        </div>

        <div className="space-y-1.5">
          <label className="block text-xs font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>Password</label>
          <div className="relative">
            <input
              type="password"
              value={password}
            ...
            <div className="flex gap-1 mt-1">
              {[1,2,3,4].map((level) => (
                <div
                  ...
                  style={{ background: strength >= level ? strengthColor : "rgba(255,255,255,0.1)" }}
                />
              ))}
            </div>
          )}
      ...
      <p className="text-center text-sm mt-6" style={{ color: "rgba(255,255,255,0.4)" }}>
        ...
        <button onClick={() => onSwitch("signin")} className="font-medium" style={{ color: "#38E0C3" }}>
          Sign in
        </button>
      </p>
    </motion.div>
  ...
  return (
    <motion.div key="forgot" {...slide}>
      ...
      {sent ? (
        ...
        <>
          <h2 className="text-[22px] font-semibold text-white mb-1">Reset password</h2>
          <p className="text-sm mb-6" style={{ color: "rgba(255,255,255,0.45)" }}>
...
function CheckEmail({ onSwitch }: { onSwitch: (t: Tab) => void }) {
  return (
    <motion.div key="check-email" {...slide} className="text-center py-4">
      ...
      <button onClick={() => onSwitch("signin")} className="text-sm font-medium" style={{ color: "#38E0C3" }}>
        Back to sign in
      </button>
    </motion.div>
  );
...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-dvh flex" style={{ background: "#0a1628" }}>
      {/* Background orbs */}
      <div style={{ position: "absolute", width: "600px", height: "600px", borderRadius: "50%", background: "rgba(56,224,195,0.10)", top: "-200px", left: "-150px", pointerEvents: "none" }} />
      <div style={{ position: "absolute", width: "400px", height: "400px", borderRadius: "50%", background: "rgba(99,179,255,0.07)", bottom: "-100px", right: "-80px", pointerEvents: "none" }} />
      ...
      <div className="hidden lg:flex flex-col justify-between w-[42%] relative overflow-hidden p-12"
        ...
        <div style={{ position: "absolute", width: "350px", height: "350px", borderRadius: "50%", background: "rgba(56,224,195,0.07)", top: "30%", left: "-80px", pointerEvents: "none" }} />
        ...
        <div className="relative z-10">
          <p className="text-[11px] font-medium tracking-widest uppercase mb-6" style={{ color: "rgba(56,224,195,0.7)" }}>
            Trusted by 10,000+ students
          </p>
          <blockquote className="text-2xl font-display text-white leading-snug mb-4">
            "I went from 4 hours making notes to 20 minutes. StudyLM is insane for exam prep."
          </blockquote>
          ...
          <div className="flex flex-wrap gap-2 mt-8">
            {["AI Chat", "Flashcards", "Quizzes", "Mind Maps", "Study Guides"].map((f) => (
              <span
                key={f}
                className="px-3 py-1 rounded-full text-xs font-medium"
                style={{ background: "rgba(56,224,195,0.08)", border: "0.5px solid rgba(56,224,195,0.2)", color: "rgba(255,255,255,0.65)" }}
              >
                {f}
              </span>
            ))}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 relative z-10">
        ...
        <div className="flex items-center gap-2.5 mb-10 lg:hidden">
          <img src="/logo.jpg" alt="StudyLM" className="w-8 h-8 rounded-xl object-cover" />
          <span className="font-display text-lg text-white">StudyLM</span>
        </div>
